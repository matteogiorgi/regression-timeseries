...
\begin{lstlisting}[style=rstyle, caption={Variance Inflation Factors for the regression model}, label={lst:vif}]
library(performance)
...
\begin{lstlisting}[style=routput, caption={Variance Inflation Factors (VIF) for selected regressors}, label={lst:vif-output}]
Low Correlation
...
The VIF results confirm the absence of severe multicollinearity for most regressors in the model.
Variables such as \texttt{GrLivArea}, \texttt{LotArea}, \texttt{FullBath}, \texttt{GarageArea}, and \texttt{GarageCars} exhibit VIF values between 2 and 5, which is commonly interpreted as an acceptable level of correlation.

A second group of variables --- notably \texttt{FirstFlrSF}, \texttt{TotalBsmtSF}, \texttt{YearBuilt}, \texttt{KitchenQual}, \texttt{ExterQual}, and \texttt{BsmtQual} --- shows VIF values between 5 and 8.5, indicating moderate multicollinearity, consistent with the fact that these variables describe closely related characteristics of a dwelling (size or quality dimensions).

The only regressor exhibiting high multicollinearity is \texttt{Neighborhood}, with VIF $\approx 41$.
This result is expected: the variable expands into more than 25 dummy variables, creating strong dependence among categories and absorbing substantial variation.

Importantly, while high VIF values increase the standard errors of the affected coefficients, they do not bias the OLS estimator.
Given the relatively large sample size and the strong theoretical justification for including neighbourhood fixed effects, we retain all regressors in the model.
