...
\subsection{Structural Stability: Chow Test around the 2008 Financial Crisis}
...
% The period covered by the dataset includes the years of the 2008 financial crisis, which may have affected the housing market and, consequently, the relationship between the explanatory variables and \texttt{SalePrice}.
% To assess whether the regression coefficients are stable over time, we perform a Chow test for a structural break with respect to the year of sale (\texttt{Yr.Sold}).

% We split the sample into two subperiods:
% \begin{itemize}
%     \item \textbf{pre-crisis period}: transactions in 2006--2007;
%     \item \textbf{crisis and post-crisis period}: transactions in 2008--2010.
% \end{itemize}
...
\subsection{Functional Form Misspecification: RESET Test}

We now investigate whether the linear functional form assumed in our baseline specification is adequate.
A classical diagnostic tool for this purpose is the Ramsey RESET test, which evaluates whether nonlinear combinations of the fitted values possess explanatory power beyond the original regressors.
A low p-value indicates rejection of the null hypothesis of correct functional form.

In our case, the RESET test produces a very small p-value, leading us to reject the null hypothesis of linearity.
...
As discussed in Section~\ref{...}, the Chow test shows no evidence of a structural break around the 2008 financial crisis, which we consider the most plausible breakpoint.
Therefore, it is reasonable to attribute the nonlinearity detected by the RESET test to omitted relevant terms, such as interactions or nonlinear transformations of existing regressors.
...
Although \texttt{R} initially treats these variables as unordered categorical factors, they possess a meaningful ranking from ``Poor'' to ``Excellent''.
...
To address additional sources of nonlinearity, we also introduce two economically motivated interaction terms:
\[
    \texttt{GrLivArea} \times \texttt{OverallQual},
    \qquad
    \texttt{GarageArea} \times \texttt{GarageCars}.
\]

These interactions have a clear economic interpretation.
...
Similarly, the value contributed by a large garage depends on its capacity: a wide two-car garage is typically worth more than a very large one-car garage.
...
This choice is supported by economic reasoning: in a log–log specification, coefficients represent elasticities, i.e., percentage changes in \texttt{SalePrice} associated with percentage changes in the regressors.
This helps capture diminishing marginal effects.
...
To assess whether the linear functional form assumed in our baseline model is appropriate, we apply the Ramsey RESET test.
The test evaluates whether higher-order polynomials of the fitted values carry additional explanatory power, which would indicate misspecification due to omitted nonlinearities or functional form errors.

The RESET test on the baseline linear model rejects the null hypothesis of correct specification, with a p-value essentially equal to zero.
This suggests that the model suffers from functional form misspecification.
As discussed previously, the Chow test did not reveal evidence of structural breaks, so nonlinearity is unlikely to arise from instability over time.
Instead, the RESET result points towards the omission of nonlinear transformations or interaction effects between regressors.

A relevant issue concerns the ordinal regressors in the dataset—\texttt{OverallQual}, \texttt{KitchenQual}, \texttt{ExterQual}, and \texttt{BsmtQual}—which carry an inherent ordering from ``Poor'' to ``Excellent''.
Treating them as ordered factors allows the model to capture nonlinear patterns between successive levels rather than interpreting the quality categories as unrelated dummy variables.
The variable \texttt{OverallQual}, despite being coded numerically from 1 to 10, is also ordinal in nature and is therefore transformed accordingly.

To address the functional misspecification, we consider two adjustments:
(i) a log--log model, which provides elasticities and captures diminishing marginal effects; and
(ii) the addition of economically meaningful interaction terms, such as
\(\texttt{GrLivArea} \times \texttt{OverallQual}\)
and
\(\texttt{GarageArea} \times \texttt{GarageCars}\).
These interactions account for cases where the effect of size depends strongly on quality, a relationship supported by housing market intuition.
...
% The Chow-type test implemented via \texttt{sctest} yields a test statistic $f(\text{efp}) = 1.30$ with a p-value of 0.8746.
% Therefore, we fail to reject the null hypothesis of parameter stability at any conventional significance level: there is no statistical evidence of a structural break in the relationship between \texttt{SalePrice} and the selected regressors around the 2008 financial crisis.

% The separate subsample regressions provide additional insight.
% The pre-2008 model attains an adjusted $R^2$ of approximately 0.844, while the post-2008 model reaches an adjusted $R^2$ close to 0.897, indicating a somewhat better fit in the later period.
% However, the signs and magnitudes of the main structural coefficients (such as those on \texttt{GrLivArea}, \texttt{TotalBsmtSF}, \texttt{LotArea}, and \texttt{OverallQual}) remain broadly consistent across the two subsamples.
% Taken together, these results suggest that, although the crisis may have affected the overall volatility of the housing market, the underlying price determinants captured by our model are structurally stable over time.
...
To formally assess this, we conduct the White test for heteroskedasticity.
...
The White test produces a very small p-value, leading us to reject the null hypothesis of homoskedasticity.
We therefore conclude that heteroskedasticity is present in the model.

A standard remedy is to compute heteroskedasticity-robust standard errors, such as the HAC (heteroskedasticity and autocorrelation consistent) estimator of the covariance matrix.
...
The Durbin--Watson test, in particular, is designed for time-series or panel data where residuals may be correlated with their past values.
...
Nevertheless, computing the Durbin--Watson statistic on the model's residuals yields a value of approximately 1.84, which is close to the theoretical value of 2 under the null hypothesis of no first-order autocorrelation.
